...
@implementation C4Sound
+(void)load {
	if(VERBOSELOAD) printf("C4Sound\n");
}

+(C4Sound *)systemSound:(NSString *)soundName {
	return [[[C4Sound alloc] initWithSystemSound:soundName] autorelease];
}

+(C4Sound *)soundWithName:(NSString *)fileName andType:(NSString *)extension {
	return [[[C4Sound alloc] initWithName:fileName andType:extension] retain];
}
